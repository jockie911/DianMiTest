...
//    }
    ...
    public static Bitmap stringToBitmap(String text, int size, int color, Typeface font, int bgColor) {
        ...
        float space =  BaseApp.getContext().getResources().getDimensionPixelSize(R.dimen.DIMEN_2PX);  //背景padding

        int textHeight = (int) getTextHeight(p);    //字的高度
        int pixLen = (int) (getFontHeight(p) + 2* space);//正方形边长

        int offsetY =   BaseApp.getContext().getResources().getDimensionPixelSize(R.dimen.DIMEN_2PX);    //y方向上的偏移  手动调整
    ...
    public static float getLineSpaceHeight(){
        return   BaseApp.getContext().getResources().getDimensionPixelSize(R.dimen.DIMEN_4PX);
    }
    ...
    public static float getCenterY(Paint paint){
        int offset =  BaseApp.getContext().getResources().getDimensionPixelSize(R.dimen.DIMEN_4PX);
